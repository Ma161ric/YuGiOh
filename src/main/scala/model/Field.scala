...
case class Field(size: Int, round: Int = 1, deck: Int = 40, hand: Hand, fightField: FightField, player1: Player, player2: Player):
  ...
  def playerStatsRow(cellWidth: Int = 3, cellNum: Int = 3, player: Player) =
    "| " + playerName(cellWidth, player.toString) + playerLp(cellWidth, player.getLp) + (" " * cellWidth) * (cellNum-4) + " " * (cellNum-4) + "|" + eol


  def mesh(cellWidth: Int = 10, cellNum: Int = size, player1: Player, player2: Player): String =
